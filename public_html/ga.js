...
function newGeneration(aKarts, oMap) {
    ...
    for(var i=2; i<aKarts.length; i++){
        ...
        var newMario;
        if(partnerA.fitness > partnerB.fitness) partnerB = crossOver(partnerA,partnerB);
       	else partnerA = crossOver(partnerB, partnerA);
        
       	if(partnerA.fitness > partnerB.fitness) newMario = partnerA;
        else newMario = partnerB;

        newMario.brain.mutate(mutationRate, 0.3);
